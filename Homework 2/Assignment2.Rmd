...
alpha <- 0.05
t <- qt(1-alpha/2, df = n-2)
SE <- sqrt(sum((y - beta_0 - beta_1 * x)^2) / (n-2)) * sqrt(1/n + x_mean^2 / Sxx)
CI <- c(beta_0 - t * SE, beta_0 + t * SE)
cat("95% confidence interval for beta_0 is: ", CI)
```
#### (c)
```{r}
# 95% confidence interval for beta_1 (slope)
SE <- sqrt(sum((y - beta_0 - beta_1 * x)^2) / (n-2)) / sqrt(Sxx)
CI <- c(beta_1 - t * SE, beta_1 + t * SE)
cat("95% confidence interval for beta_1 is: ", CI)
```
#### (d)
...
```
Since the p-value = 0.1368 > 0.05, we do not reject the null hypothesis that beta_0 = 0. There is not enough evidence to suggest that the analytical result is 0 when there is no calcium in the solution.
...
```
Since the p-value = 0.34451 > 0.05, we do not reject the null hypothesis that beta_1 = 1. There is not enough evidence to suggest that the slope of the linear model is 1.
...
```{r}
# Recalculate the 95% confidence interval for beta_1
SE <- sqrt(sum((y - beta_1 * x)^2) / (n-1)) / sqrt(Sxx)
CI <- c(beta_1 - t * SE, beta_1 + t * SE)
cat("95% confidence interval for beta_1 when beta_0 = 0 is: ", CI)
```
